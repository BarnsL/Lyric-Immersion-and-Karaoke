...
## v1.1.x — 2026-06-29 session (TICKET-130 → 135)
...
- **TICKET-134 — GPU renderer: full port to match CPU "exactly but higher fps"** 🔴 (user chose the full port). The GL belt is a naive draw-everything reimpl: (a) too many lines = "whole screen full"; (b) highlight lost in the clutter; (c) clicking can crash the child, leaving its opaque-black frame = "black background" (parent must destroy the dead child window + restore Tk); (d) no settings parity (opacity/position/font-scale not applied); (e) belt freezes during a parent stall (child should self-extrapolate `_pos`). FIX = port the CPU `_ticker_update` belt (spawn/fill budgets, single-row layout, line limiting). Verify OFFLINE via `scratchpad gpu/probe_live.py` (full-screen `--width 1920 --height 1080` + self-capture), NOT live screenshots.
- **TICKET-135 — run Shazam recognize in a SUBPROCESS (smooth highlights)** 🔴 recognize is a thread → shares the GIL → stalls the render (~150-475ms frames) during each ~4s capture. Robust fix for fully-smooth highlights: run it as a child process (mirror the `--gpu-renderer-child` pattern), result over a pipe. Would also let recal poll often without stutter.
