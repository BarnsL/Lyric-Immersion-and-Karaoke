...
## v1.1.77 — build hardening (TICKET-175 — a broken AI stack could ship silently)

- **TICKET-175 — faster-whisper silently broke in every packaged build v1.1.74→v1.1.76** 🟢. RCA: the spec bundles the optional AI stack (faster-whisper + ctranslate2 + **PyAV** + tokenizers) from a vendored `./.deps` folder with `pathex=[".deps"]`, and `collect_all("av")` searches BOTH `.deps` and the build env. `.deps` held **av 17.1.0** while the env had **av 18.0.0** (also ctranslate2 4.8.0 vs 4.8.1, tokenizers 0.23.1 vs 0.22.2) — a version-SKEWED mix of Python modules + FFmpeg DLLs got bundled, so `import av` (→ `import faster_whisper` → `align.available()`) died at runtime with an `av._core` error. That disabled generate-by-ear, sync-by-listening AND the whole wrong-lyrics reject path (TICKET-174 depends on decide-by-ear, which needs whisper) — invisibly: the app just showed "needs faster-whisper" hints and every listen feature degraded, nothing in the log. `.deps` is gitignored, so any build machine could reintroduce the skew. TWO guards, both verified: **(1) pre-build** `scripts/check_build_deps.py` compares `.deps` vs build-env versions of the native stack AND flags duplicate dist-info dirs (a `pip install --upgrade --target` leaves the old one behind → the bundle becomes a coin-flip); build.bat step 1b fails the build on skew. **(2) post-build** `main.py --selftest --out FILE` runs BEFORE any GUI init in the FROZEN exe, imports av + faster-whisper, checks `align.available()`, writes a one-line verdict, and `os._exit(0/1)`; build.bat step 2b runs it via `Start-Process -Wait -PassThru` (no window) and fails the build if the finished exe's AI stack is broken. Also **fixed this machine's `.deps`** (nuked + reinstalled pinned to env: av 18.0.0 / ct2 4.8.1 / fw 1.2.1 / tok 0.22.2) so it's deterministic, not collect-order luck. Verified: check catches the skew (exit 1) and passes when consistent (exit 0); frozen `--selftest` returns "av + faster-whisper import and align.available() is True" (exit 0) on the real bundle. NOTE: v1.1.77 was RELEASED before this hardening; its binary works (env-av won by collect order), so no re-release — the guards protect FUTURE builds.
